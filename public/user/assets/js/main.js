...
//Remove from cart
function cartProductRemove(CartId, ProductUniqueId, UserId) {
  $.ajax({
    method: "post",
    url: "/removeFromCart",
    data: { Cart: CartId, Product_To_Remove: ProductUniqueId, UserId: UserId },
    success: (response) => {
      if (!response.Empty) {
...
function AddQuantity(CartId, ProductUniqueId, UserId, ChangeCount) {
  const limit = 10;
  let ProductQuantity = parseInt(
    document.getElementById(ProductUniqueId).innerHTML
  );
  if (ProductQuantity >= limit) {
    let Cartid = document.getElementById("plusbtn" + ProductUniqueId).value;
    Cartid.limit = 10;
  }
  $.ajax({
    url: "/changeQuantity",
...
function ReduceQuantity(CartId, ProductId, UserId, ChangeCount) {
  const limit = 1;
  let ProductQuantity = parseInt(document.getElementById(ProductId).innerHTML);
  if (ProductQuantity <= limit) {
    let Cartid = document.getElementById("minusbtn" + ProductId).innerHTML;
    Cartid.limit = 1;
  }
  $.ajax({
    url: "/changeQuantity",
...
function FormSettings() {
  New_AddressBtn = document.getElementById("New_AddressBtn");
  if (New_AddressBtn.checked) {
    $("#AddressForm").attr("style", "display:block");
  } else if (!New_AddressBtn.checked) {
    $("#AddressForm").attr("style", "display:none");
  }
}

function RemoveDefaultAddress(AddressUniqueId) {
  $.ajax({
    url: "/RemoveDefault",
    method: "post",
    data: { Address: AddressUniqueId },
    success: (Response) => {
      if (Response.Status) {
        $("#AddressCard").load(window.location.href + " #AddressCard");
      } else {
        alert("error in make default");
...
function MakeDefaultAddress(AddressUniqueId) {
  $.ajax({
    url: "/MakeDefault",
    method: "post",
    data: { Address: AddressUniqueId },
    success: (Response) => {
      if (Response.Status) {
        ...
        alert("maximum 1 default address");
        $("#AddressCard").load(window.location.href + " #AddressCard");
      } else {
        alert("error in making default");
        $("#AddressCard").load(window.location.href + " #AddressCard");
      }
    },
  });
...
pageSize = 9;
pageNo = 1;
var pageCount = Math.ceil($(".paginationcontent").length / pageSize);
showPage = function (page) {
  ...
  $(".paginationcontent").each(function (n) {
    ...
    if (page==1 && $(".paginationcontent").length <= 9) {
      ...
      $(".pagination .page-link-next").hide();
      $(".pagination .page-link-prev").show();
    } else if (page === parseInt(1) && $(".paginationcontent").length > 9) {
      $(".pagination .page-link-next").show();
      $(".pagination .page-link-prev").hide();
    } else if (page <= pageCount) {
      $(".pagination .page-link-next").show();
      $(".pagination .page-link-prev").show();
...
showPage(pageNo);
$(".pagination .page-link-next").click(function () {
  pageNo++;
  showPage(parseInt(pageNo));
});
$(".pagination .page-link-prev").click(function () {
  pageNo--;
  showPage(parseInt(pageNo));
});
...
function filterMainCategory(MainCatId) {
  ...
  if (checkbox.checked) {
    ...
    $.ajax({
      url: "/allProducts?product=" + MainCatId,
      method: "get",
      success: () => {
        alert("done");
      },
    ...
    $(".CategoryInputs").attr("disabled", false);
    $.ajax({
      url: "/allProducts",
      method: "get",
      success: () => {
